...
export const getFormDefaultValues = (record) => {
  return {
    ...
    team: record?.team || "",
    //carrier: record?.carrier && {value: String(record?.carrier),label: String(record?.carrier),},
    ...
    //status: record?.status && {value: String(record?.status),label: String(record?.status),},
    status: record?.status && JSON.parse(record?.status) || "",
    expiry_date: isValidDate(record?.expiry_date) && new Date(record?.expiry_date),
