...
const RecordsList = () => {
  ...
  const listUrl = ecom_endpoints?.products?.invoices?.getRecords();

  const deleteUrl = ecom_endpoints?.products?.invoices?.deleteRecord(
    record?.id
  );
  const ReStoreRecord = ecom_endpoints?.products?.invoices?.reStoreRecord(
    record?.id
  );
  ...
  return (
    <>
      <PageSubTitle title={pageTitle}>
        <Link href={ADMIN_PATHS?.FINANCE?.INVOICES?.ADD}>
