...
import { getInvoiceStages, getCarriers } from "@/helper/EcomActions";
import { formSchema } from "./formSchema";
import { getFormDefaultValues } from "./formDefaultValues";
import SalesOrdersModal from "./SalesOrdersModal";
...
export default function Form({ record }) {
  ...
  const [salesOrdersModal, setSalesOrdersModal] = useState(false);
  ...
  const addRecordUrl = ecom_endpoints.products.invoices.create;
  // PAGE RELATED - ending
  ...
  const onSubmit = async (formData) => {
    try {
      const body = {
        ...
        sales_person_id: formData?.sales_person_id?.value,
        ...
        sales_order_id: formData?.sales_order_id?.value || 0,
        sales_order_no: formData?.sales_order_id
          ? JSON.stringify(formData?.sales_order_id)
          : "",
  ...
  useEffect(() => {
    ...
    const fetchSalesOrders = async () => {
      ...
      try {
        ...
        if (response?.status === 200) {
          const formatedData = response?.data.map(({ id, sales_order_no }) => ({
            value: String(id),
            label: String(sales_order_no),
          }));
          setSalesOrders(formatedData);
        }
      } catch (error) {
        console.log(error);
      }
  ...
  // RESET RECORD WITH SALE ORDER - starting
  useEffect(() => {
    if (selectedSaleOrder) {

      selectedSaleOrder.sale_order_no = JSON.stringify({
        value: String(selectedSaleOrder?.id),
        label: String(selectedSaleOrder?.sales_order_no),
      });
      methods.reset(getFormDefaultValues(selectedSaleOrder));

      if (selectedSaleOrder?.items) {
        const xItems = JSON.parse(selectedSaleOrder?.items);
        setItems(xItems);
      }

      setSummary({
        subtotal: selectedSaleOrder?.subtotal,
        discount: selectedSaleOrder?.discount,
        tax: selectedSaleOrder?.tax,
        grand_total: selectedSaleOrder?.grand_total,
      });
    }
  }, [selectedSaleOrder, methods]);
  // RESET RECORD WITH SALE ORDER - ending
    ...
    <div className="bg-white shadow-lg p-6">
      ...
      >
        <div className="space-y-5">
          <h4 className="display5 font-semibold">Invoice Information</h4>
          <div className="grid md:grid-cols-2 container mx-auto !p-0 gap-4">
            <div className="space-y-4">
              <SelectControl
                ...
                onMenuOpen={() => setSalesOrdersModal(true)}
              />
              <SelectControl
                title="Invoice Owner"
        ...
        <div className="space-y-5 !overflow-x-auto">
          <h4 className="display5 font-semibold">Invoice Items</h4>
        ...
        <div>
          <div className="flex justify-center lg:justify-between col-span-6 space-x-4 mt-6">
            <Button
              ...
              variant="outline"
              type="button"
              onClick={() => router.push(ADMIN_PATHS?.FINANCE?.INVOICES?.LIST)}
            >
              View All Invoices
            </Button>
            <Button
      ...
      {salesOrdersModal && (
        <SalesOrdersModal
          setSalesOrdersModal={setSalesOrdersModal}
