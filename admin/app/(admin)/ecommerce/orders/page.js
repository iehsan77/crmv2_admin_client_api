...
const RecordsList = () => {
  ...
  const listUrl = ecom_endpoints?.products?.salesOrders?.getRecords();

  const deleteUrl = ecom_endpoints?.products?.salesOrders?.deleteRecord(record?.id);
  const ReStoreRecord = ecom_endpoints?.products?.salesOrders?.reStoreRecord(record?.id);
  ...
  return (
    <>
      <PageSubTitle title={pageTitle}>
        <Link href={ADMIN_PATHS?.ECOMMERCE?.ORDERS?.ADD}>
