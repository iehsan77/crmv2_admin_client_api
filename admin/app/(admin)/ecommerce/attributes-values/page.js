...
const RecordsList = () => {
  ...
  const listUrl = ecom_endpoints?.products?.attributeValues?.list;
  const deleteUrl = ecom_endpoints?.products?.attributeValues?.deleteRecord(
    record?.id,
    vendor?.vendor_website_id
  );
  const ReStoreRecord =
    ecom_endpoints?.products?.attributeValues?.reStoreRecord(
      record?.id,
      vendor?.vendor_website_id
    );
