...
const RecordsList = () => {
  ...
  const pageTitle = "Product Warehouses";
  const listUrl = ecom_endpoints?.products?.warehouses?.list;
  const deleteUrl = ecom_endpoints?.products?.warehouses?.deleteRecord(
    record?.id,
    vendor?.vendor_website_id
  );
  const ReStoreRecord = ecom_endpoints?.products?.warehouses?.reStoreRecord(
    record?.id,
    vendor?.vendor_website_id
  );
