...
const RecordsList = () => {
  ...
  const pageTitle = "Product Tags";
  const listUrl = ecom_endpoints?.products?.tags?.list;
  const deleteUrl = ecom_endpoints?.products?.tags?.deleteRecord(
    record?.id,
    vendor?.vendor_website_id
  );
  const ReStoreRecord = ecom_endpoints?.products?.tags?.reStoreRecord(
    record?.id,
    vendor?.vendor_website_id
  );
