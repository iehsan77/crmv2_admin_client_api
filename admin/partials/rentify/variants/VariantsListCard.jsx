...
export default function VariantsListCard({ record }) {
  const [status, setStatus] = useState(record?.is_used);

  const { showDrawer } = useDrawer();

  const { updateVariant, deleteVariant } = useVariantsStore();
  ...
  const handleStatusChange = async (value) => {
    ...
    try {
      const api_url =
        value === 1
          ? rentify_endpoints?.rentify?.variants?.SelectForTenant
          : rentify_endpoints?.rentify?.variants?.UnselectForTenant;

      const response = await POST(api_url, body);

      if (response?.status === 200) {
        const updatedVariant = { ...record, is_used: value };
    ...
    setOnConfirm(async () => {
      try {
        const response = await GET(
          rentify_endpoints?.rentify?.variants?.delete(id)
        );
                ...
                <Button
                  ...
                  onClick={() =>
                    showDrawer({
                      title: "Add Variants",
                      size: "xl",
                      content: <VariantsForm record={record} />,
                    })
                  }
                >
