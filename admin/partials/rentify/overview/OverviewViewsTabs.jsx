...
export default function OverviewViewsTabs({
  availableTabs = [],
  initialTabs = [],
  ...
  return (
    <div className="flex items-center justify-between border-t bg-[#F5F9FF]">
      {/* Scrollable Tabs - FIXED */}
      <div className="flex-1 overflow-x-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent">
        <div className="flex min-w-0">
          {" "}
          {/* Changed from min-w-max to min-w-0 */}
          {tabs.map((tab) => (
            <div
              key={tab.value}
              className={`flex-shrink-0 flex items-center justify-between px-4 py-3 cursor-pointer whitespace-nowrap text-sm border-x border-gray-200 ${
                isTabActive(tab.value)
                  ? "bg-white text-black border-b-white relative z-10"
                  : "bg-[#F5F9FF] text-muted-foreground"
              }`}
              style={{ minWidth: "160px", maxWidth: "200px" }} // Fixed widths for consistent sizing
              onClick={() => {
            ...
            >
              ...
              {allowClose && tab.value !== initialTabs[0]?.value && (
                <button
                  ...
                  className="ml-2 flex-shrink-0 text-gray-500 hover:text-red-500 transition-colors"
                >
                  <X className="w-4 h-4" />
                </button>
              )}
      ...
      {showAddView && (
        <Button
          variant="ghost"
          className="flex-shrink-0 flex items-center gap-1 text-sm text-muted-foreground bg-white rounded-none border-l border-gray-200 px-4 py-3 h-auto"
          onClick={() =>
            showDrawer({
              title: "Add View",
              size: "xl",
...
function DrawerContent({ availableTabs }) {
  const { tabs, addTab, removeTab, getMaxTabs } = useVehiclesViewTabsStore();
