...
import { INDUSTRIES, USER_ROLES } from "@/constants/general_constants";
...
export default function Kanban() {
  ...
  const kanbanData = useAccountsStore((s) => s.kanbanData);
  const setKanbanData = useAccountsStore((s) => s.setKanbanData);
  const kanbanDataLoading = useAccountsStore((s) => s.kanbanDataLoading);
  const fetchKanbanRecords = useAccountsStore((s) => s.fetchKanbanRecords);
  ...
  const getListByStatus = (data, statusId) => {
    if (Array.isArray(data)) {
      ...
      const accounts = statusObj?.accounts || [];
    ...
    if (Array.isArray(kanbanData)) {
      newData = kanbanData.map((col) => {
        if (String(col.id) === String(sourceStatus)) {
          const newAccounts = [...(col.accounts || [])];
          newAccounts.splice(sourceIdx, 1);
          return { ...col, accounts: newAccounts };
        }
        if (String(col.id) === String(destinationStatus)) {
          const newAccounts = [...(col.accounts || [])];
          newAccounts.splice(destinationIdx, 0, {
            ...recordToMove,
  ...
  const updateStatus = async (id, status_id) => {
    try {
      console.log("📤 API Account:", { id, status_id });
      const response = await POST(crm_endpoints?.crm?.accounts?.updateStatus, {
        id,
        status_id,
      });
