...
import { useAttachmentsStore } from "@/stores/rentify/affiliates/useAffiliateStore";
  ...
  const onSubmit = async (formData) => {
    try {
      ...
      const response = await POST(
        rentify_endpoints?.rentify?.affiliates?.saveAttachements,
        body
      );

      hideDrawer();
      if (response?.status === 200) {
        fetchAttachments(affiliate_id);
        /*
        if (record?.id) {
          updateAttachment(response?.data);
        } else {
          saveAttachment(response?.data);
        }
        */
      } else {
