              ...
              <div className="grid sm:grid-cols-6 gap-x-6 gap-y-8">
                ...
                <div className="col-span-6 sm:col-span-3">
                  <DatePickerInput
                    ...
                    minDateTime={
                      watch("start_time")
                        ? new Date(watch("start_time")).toISOString()
...
export const FormSchema = z
  ...
  // ✅ Ensure end_time > start_time (robust comparison)
  .refine(
    (data) => {
      ...
      // handle invalid or time-only strings gracefully
      if (isNaN(start.getTime()) || isNaN(end.getTime())) {
        try {
          // fallback if only time values like "09:00"
          const today = new Date().toISOString().split("T")[0];
          const startAlt = new Date(`${today}T${data.start_time}`);
          const endAlt = new Date(`${today}T${data.end_time}`);
          if (isNaN(startAlt.getTime()) || isNaN(endAlt.getTime())) return true;
