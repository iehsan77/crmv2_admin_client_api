...
function KanbanCard({ record = {} }) {
  ...
  const relatedTo_label = record?.related_to;
      ...
      <div className="p-4 border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow">
        ...
        <div className="w-full md:w-4/5_ flex flex-col gap-3">
          ...
          <div className="gap-x-6 gap-y-2 text-sm text-gray-700">
            <div>
              <InfoItem label="Subject" value={record?.title} />
            </div>
            <div>
...
export default function Kanban() {
  ...
  const kanbanData = useMeetingsStore((s) => s.kanbanData);
  const setKanbanData = useMeetingsStore((s) => s.setKanbanData);
  const kanbanDataLoading = useMeetingsStore((s) => s.kanbanDataLoading);
  const fetchKanbanRecords = useMeetingsStore((s) => s.fetchKanbanRecords);
  ...
  const getListByStatus = (data, statusId) => {
    if (Array.isArray(data)) {
      ...
      const meetings = statusObj?.meetings || [];
    ...
    if (Array.isArray(kanbanData)) {
      newData = kanbanData.map((col) => {
        if (String(col.id) === String(sourceStatus)) {
          const newMeetings = [...(col.meetings || [])];
          newMeetings.splice(sourceIdx, 1);
          return { ...col, meetings: newMeetings };
        }
        if (String(col.id) === String(destinationStatus)) {
          const newMeetings = [...(col.meetings || [])];
          newMeetings.splice(destinationIdx, 0, {
            ...recordToMove,
  ...
  const updateStatus = async (id, status_id) => {
    try {
      console.log("📤 API Meeting:", { id, status_id });
      const response = await POST(crm_endpoints?.crm?.meetings?.updateStatus, {
        id,
        status_id,
      });
