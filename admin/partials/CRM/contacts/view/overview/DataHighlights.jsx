...
import { formatDateTime, getMocDataLabelByValue } from "@/helper/GeneralFunctions";
import { LEADS_STATUS_OPTIONS } from "@/constants/crm_constants";

export default function DataHighlights() {

  const record = useContactsStore((s) => s.recordDetails);

  const stats = [
    {
      label: "Created Date",
      value: formatDateTime(record?.createdon || record?.created_at),
      icon: "uil:calendar-alt",
      link: "#",
    },
    {
      label: "Contact Status",
      value: getMocDataLabelByValue(LEADS_STATUS_OPTIONS, record?.status_id),
