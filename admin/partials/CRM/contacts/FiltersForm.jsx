...
const FiltersForm = ({ isDrawer = false, showMoreButton = true }) => {
  ...
  const setPage = useContactsStore((s) => s.setPage);
  ...
  useEffect(() => {
    ...
    if (!filters || filters.length === 0) {
      setFilters(CONTACTS_INITIAL_FILTERS);
      setActiveFilter(CONTACTS_INITIAL_FILTERS[0]?.value || "");
    }
      ...
      {filterMapValues.includes("postal_code") && (
        <span>
          <TextInput
            ...
            onChange={(val) => updateValue("postal_code", val.target.value)}
          />
        </span>
      )}
        ...
        {filters.length > 0 && (
          <>
            ...
            <ClearFilters
              ...
              useFiltersStore={useContactsFiltersStore}
            />
          </>
        )}
        {showMoreButton && (
              ...
              showDrawer({
                ...
                content: (
                  <FiltersDrawerContent
                    availableFilters={CONTACTS_AVAILABLE_FILTERS}
                    useFiltersStore={useContactsFiltersStore}
                  />
                ),
              })
