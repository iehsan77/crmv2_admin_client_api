...
export default function Kanban() {
  ...
  const { updateCall } = useCallsStore();
  ...
  const kanbanData = useCallsStore((s) => s.kanbanData);
  const setKanbanData = useCallsStore((s) => s.setKanbanData);
  const kanbanDataLoading = useCallsStore((s) => s.kanbanDataLoading);
  const fetchKanbanRecords = useCallsStore((s) => s.fetchKanbanRecords);
    ...
    if (Array.isArray(kanbanData)) {
      newData = kanbanData.map((col) => {
        if (String(col.id) === String(sourceStatus)) {
          const newCalls = [...(col.calls || [])];
          newCalls.splice(sourceIdx, 1);
          return { ...col, calls: newCalls };
        }
        if (String(col.id) === String(destinationStatus)) {
          const newCalls = [...(col.calls || [])];
          newCalls.splice(destinationIdx, 0, {
            ...recordToMove,
  ...
  const updateStatus = async (id, status_id) => {
    try {
      console.log("📤 API Call:", { id, status_id });
      const response = await POST(crm_endpoints?.crm?.calls?.updateStatus, {
        id,
        status_id,
      });
