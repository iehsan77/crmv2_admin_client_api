...
export default function RelatedAccountsList() {
  ...
  const records = useAccountsAssociationStore((s) => s.records);
  const loading = useAccountsAssociationStore((s) => s.loading);
  const fetchRecords = useAccountsAssociationStore((s) => s.fetchRecords);
  ...
  const columns = useMemo(
    () => Columns({ currentPage, itemsPerPage }),
    [currentPage]
  );
    ...
    <Card className="gap-2">
      <CardHeader>
        <div className="flex items-center justify-between">
          <h2 className="text-base font-medium text-gray-900">Accounts</h2>
          <CardActions />
        </div>
      </CardHeader>
