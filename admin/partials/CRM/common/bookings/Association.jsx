...
import useBookingsStore, {
  useBookingsAssociationStore,
} from "@/stores/rentify/useBookingsStore";

import { useDrawer } from "@/context/drawer-context";
import { getMocDataLabelByValue, useSegment } from "@/helper/GeneralFunctions";
import { BOOKING_STATUSES } from "@/constants/rentify_constants";
...
const FormSchema = z.object({
  ...
  target_module_ids: z
    ...
    .nonempty("Please select at least one booking"),
});

const setBookingsDropdownData = (records = []) => {
  if (!Array.isArray(records)) return [];
  return records.map((item) => {
    if (item.booking_uid && item.pickup_time && item.booking_status_id) {
      return {
        label: String(
          `${item.booking_uid} - ${item.pickup_time} (${getMocDataLabelByValue(
            BOOKING_STATUSES,
...
export default function Association({ record = {} }) {
  ...
  const saveRecord = useBookingsAssociationStore((s) => s.saveRecord);
  ...
  const onSubmit = async (formData) => {
    try {
      const body = {
        source_module,
        source_module_id,
        target_module: "rentify_bookings",
        id: formData?.id,
    ...
    <FormProvider {...methods}>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-10">
        ...
        <div className="grid grid-cols-1 gap-6">
          <MultiSelectInput
            label="Bookings"
            options={setBookingsDropdownData(bookings)}
