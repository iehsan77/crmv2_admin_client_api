...
import useDealsStore, {useDealsAssociationStore} from "@/stores/crm/useDealsStore";
...
const FormSchema = z.object({
  ...
  target_module_ids: z
    ...
    .nonempty("Please select at least one deal"),
});
...
export default function Association({ record = {} }) {
  ...
  const saveRecord = useDealsAssociationStore((s) => s.saveRecord);

  const deals = useDealsStore((s) => s.records);
  const fetchDeals = useDealsStore((s) => s.fetchRecords);
  const dealsLoading = useDealsStore((s) => s.recordsLoading);
    ...
    <FormProvider {...methods}>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-10">
        ...
        <div className="grid grid-cols-1 gap-6">
          <MultiSelectInput
            label="Deals"
            options={getDropdownFormattedData(deals)}
