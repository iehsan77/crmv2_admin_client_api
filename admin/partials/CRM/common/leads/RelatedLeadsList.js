...
export default function RelatedLeadsList() {
  ...
  const records = useLeadsAssociationStore((s) => s.records);
  const loading = useLeadsAssociationStore((s) => s.loading);
  const fetchRecords = useLeadsAssociationStore((s) => s.fetchRecords);
    ...
    <Card className="gap-2">
      <CardHeader>
        <div className="flex items-center justify-between">
          <h2 className="text-base font-medium text-gray-900">Leads</h2>
          <CardActions />
        </div>
      </CardHeader>
      ...
      <CardContent>
        {loading ? (
          ...
          <>
            <DataTable columns={columns} data={currentData} useStore={useLeadsStore} />
            <Pagination
