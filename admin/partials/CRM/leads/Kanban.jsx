...
export default function Kanban() {
  ...
  const kanbanData = useLeadsStore((s) => s.kanbanData);
  const setKanbanData = useLeadsStore((s) => s.setKanbanData);
  const kanbanDataLoading = useLeadsStore((s) => s.kanbanDataLoading);
  const fetchKanbanRecords = useLeadsStore((s) => s.fetchKanbanRecords);
  ...
  const updateStatus = async (id, status_id) => {
    try {
      console.log("📤 API Lead:", { id, status_id });
      const response = await POST(crm_endpoints?.crm?.leads?.updateStatus, {
        id,
        status_id,
      });
