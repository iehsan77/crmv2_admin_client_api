...
function KanbanCard({ record = {} }) {
  ...
  const priorityLabel = getMocDataLabelByValue(
    TASKS_PRIORITIES_OPTIONS,
    record?.priority_id
  );
      ...
      <div className="p-4 border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow">
        ...
        <div className="w-full md:w-4/5_ flex flex-col gap-3">
          ...
          <div className="gap-x-6 gap-y-2 text-sm text-gray-700">
            ...
            <div>
              <InfoItem label="Related To Name" value={getName(relatedTo)} />
            </div>
            <div>
              <InfoItem label="Priority" value={priorityLabel} />
            </div>
            <div>
...
export default function Kanban() {
  ...
  const kanbanData = useTasksStore((s) => s.kanbanData);
  const setKanbanData = useTasksStore((s) => s.setKanbanData);
  const kanbanDataLoading = useTasksStore((s) => s.kanbanDataLoading);
  const fetchKanbanRecords = useTasksStore((s) => s.fetchKanbanRecords);
    ...
    if (Array.isArray(kanbanData)) {
      newData = kanbanData.map((col) => {
        ...
        if (String(col.id) === String(destinationStatus)) {
          const newTasks = [...(col.tasks || [])];
          newTasks.splice(destinationIdx, 0, {
            ...recordToMove,
  ...
  const updateStatus = async (id, status_id) => {
    try {
      console.log("📤 API Task:", { id, status_id });
      const response = await POST(crm_endpoints?.crm?.tasks?.updateStatus, {
        id,
        status_id,
      });
