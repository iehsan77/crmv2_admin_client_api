      ...
      >
        ...
        {icon && (
          <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            <Icon icon={icon} className="h-5 w-5 text-gray-400 group-focus-within:text-blue-500" />
          </div>
        )}
        ...
        <input
          type="email"
          {...props}
          className={`text-sm block w-full py-2 rounded-md bg-transparent focus:outline-none text-gray-900 placeholder-gray-400
          ${icon ? "pl-10" : "pl-3"}
          ${tooltip ? "pr-10" : "pr-3"}`}
        />
