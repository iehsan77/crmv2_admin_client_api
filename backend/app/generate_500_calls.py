"""
Script to Generate 250 Call Records for Testing (50 per status)
This script creates 50 call records for each status_id (5 statuses = 250 total)
...
# Import from app modules (works when script is in app directory or project root)
try:
    from networks.database import db
    from helpers.general_helper import get_next_id
except ImportError:
...
def generate_calls_by_status(tenant_id: int = 1, user_id: int = 1, batch_size: int = 50):
    """
    Generate 250 call records (50 per status_id) and insert them into the database
    ...
    try:
        calls_per_status = 50
        total_records = len(CALL_STATUSES) * calls_per_status
        ...
        print(f"Target: {total_records} records ({calls_per_status} per status)")
        print(f"Statuses: {CALL_STATUSES}")
        ...
        # Generate calls for each status
        for status_id in CALL_STATUSES:
            print(f"\nGenerating {calls_per_status} calls for status_id: {status_id}")
            
            for j in range(calls_per_status):
                ...
                # Override status_id to ensure even distribution
                call_record["status_id"] = status_id
                ...
                if len(batch_records) >= batch_size:
                    try:
                        ...
                        print(f"Progress: {total_inserted}/{total_records} calls inserted | {total_logs_created} activity logs created")
...
def main():
    ...
    print("\n" + "=" * 80)
    print("CALL RECORDS GENERATOR - 250 Records (50 per Status)")
    print("=" * 80)
    ...
    tenant_id = int(args[0]) if len(args) > 0 else 1
    user_id = int(args[1]) if len(args) > 1 else 1
    ...
    if not auto_confirm:
        ...
        print("\nAuto-confirmed with --yes flag. Starting generation...")
    
    # Generate the calls
    generate_calls_by_status(tenant_id=tenant_id, user_id=user_id)
